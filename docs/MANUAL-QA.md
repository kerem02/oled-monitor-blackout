...
On 2026-09-17, the 2.1.1 release build was tested on the setup that had intermittently dismissed blackout while the pointer remained on the other display. The user confirmed that the problem no longer reproduced. CI passed the expanded 96,621-assertion core suite and the 48-assertion Windows platform suite.

On 2026-09-18, a recurrence log identified an unfiltered Windows Plug and Play device notification as a second dismissal source. Version 2.1.2 no longer treats generic `WM_DEVICECHANGE` broadcasts as proof of display-topology change. The affected-system hardware regression remains open until the released build is exercised below.
...
- [ ] Connect/disconnect a non-display USB, audio, Bluetooth or storage device while blacked out: blackout remains active and the log records no topology-triggered dismissal.
